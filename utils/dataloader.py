...
from .utils import cvtColor, image_normalization, resize_image
...
class Dataset(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size // 3):
            ...
            image = image_normalization(np.array(image, dtype='float32'))
...
class LFWDataset():
    ...
    def generate(self):
        ...
        for annotation_line in self.validation_images:  
            ...
            image1 = resize_image(image1, [self.input_shape[1], self.input_shape[0]])
            image2 = resize_image(image2, [self.input_shape[1], self.input_shape[0]])
                
            image1, image2 = image_normalization(np.array(image1, np.float32)), image_normalization(np.array(image2, np.float32))
