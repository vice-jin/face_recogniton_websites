...
def test(test_loader, model, png_save_path, log_interval, batch_size):
    ...
    for batch_idx, (data_a, data_p, label) in pbar:
        ...
        # print('len(test_loader.validation_images)', len(test_loader.validation_images))
        
        # 打印
        if batch_idx % log_interval == 0:
            pbar.set_description('Test Epoch: [{}/{} ({:7.2f}%)]'.format(batch_idx * batch_size, 
                                                                        len(test_loader.validation_images),
                                                                        batch_idx * batch_size * 100.0 / len(test_loader.validation_images)))
    ...
    print('准确率: %2.5f+-%2.5f' % (np.mean(accuracy), np.std(accuracy)))
    print('最好的阙值: %2.5f' % best_thresholds)
    print('验证率: %2.5f+-%2.5f @ FAR=%2.5f' % (val, val_std, far))
    print(accuracy)
